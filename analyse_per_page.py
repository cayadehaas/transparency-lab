...
with open('information_per_page.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(
        ["FILENAME", 'PAGE', 'NBR. OF NEGATIVE SENTENCES', 'NBR. OF NEUTRAL SENTENCES',
    ...
    for root, dirs, files in os.walk(
        ...
        for file in tqdm(files):
            ...
            for page_nr in range(pdf_toread.numPages):  # iterate over the pages
                ...
                good = text.replace('˜', 'fi')  # remove ˜ and replace with fi
                new_good = good.replace('˚', 'fl')  # remove ˚ and replace with fl
                new_new_good = new_good.replace('™', "'")
                new_text = ''.join(map(lambda new_new_good: new_new_good.strip(), new_new_good.split('-'))).split()
                n_text = ' '.join(new_text)
                vraagtekens = re.compile('[?]').findall(n_text)
                VRAAGTEKENS = len(vraagtekens)

                n_sentences = re.sub(r'([a-z])([A-Z])', r'\1 \2', n_text)
                ...
                writer.writerow(
                    [file, 'BLZ.%s' % page_nr, negative_sentences, neutral_sentences, positive_sentences, NUMBERS,
                     VRAAGTEKENS])
